...
func VerifyPassword (userPassword string, givenPassword string) (bool,string){
	//check pwd given by user from pwd from db
...
func Signup() gin.HandlerFunc {
	return func(c* gin.Context){
		...
		//insert it into user collection
...
func Login() gin.HandlerFunc{
	return fnuc(c *gin.Context){
		...
		err := User.Collection.FindOne(ctx, bson.M{"email":user.Email}).Decode(&founduser)
		defer cancel()
...
func searchProduct() gin.HandlerFunc{
	...
	return func(c* gin.Context){
		...
		cursor, err := ProductionCollection.Find(ctx,bson.D{{}})
		//cursor is all JSON
		if err!=nil {
...
func SearchProductByQuery() gin.HandlerFunc{
	return func(c * gin.Context){
		...
		if err!= nil{
			c.IndentedJSON(404,"something went wrong like fetching the data")
			return 
		}
